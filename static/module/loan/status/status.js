...
define( ['base', 'dialog', 'doc','validate','area', 'all', 'css!./public/userManager'],function( base, Dialog, DOC, validate, area ){
	...
	***************************************/
	...
	var dialog = new Dialog.Dialog(),
		dialogPrompt = new Dialog.Dialog({type: 'prompt'});
		...
		$.ajax({
			...
			success: function( ret ){
				...
				if( ret.code == 0 ){
					...
					loanStatusHis = loanStatusHis.slice( 0, 2 ).concat( loanStatusHis.slice( 3 ) );
					$.each( loanStatusHis, function( k, v){
						$( statusTime.get( k ) ).append( v.gmtCreated.split( ' ' )[0] );
					})
					return;
				}
	...
	***************************************/
	...
	$( '.expressMain' ).on( 'submit', function(){
		...
		$.ajax({
			...
			success: function( ret ){
				if( ret.code == 0 ){
					dialogPrompt.show( '保存成功', ret.code || 0 );
				} else {
